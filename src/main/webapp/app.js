...
app.controller("bmtcController", function($http, $scope) {
    ...
    this.bunchingForSelectedBusStop = function() {
        ...
        console.log("http://172.16.85.32:8080/bmtc/api/GPSData?routeNo=" + this.routeName + "&direction=" + this.direction + "&date=" + this.date + "&lat=" + lat + "&lon=" + lon + "&name=" + bname);

        $http({
            method: "GET",
            url: "http://localhost:8080/bmtc/api/GPSData/1?routeNo=" + this.routeName + "&direction=" + this.direction + "&date=" + this.date + "&lat=" + lat + "&lon=" + lon + "&name=" + bname
    ...
    this.bunchingForSelectedBusStopOneWeek = function(){
            ...
            console.log("http://localhost:8080/bmtc/api/GPSData?routeNo=" + this.routeName + "&direction=" + this.direction + "&date=" + this.date + "&lat=" + lat + "&lon=" + lon + "&name=" + bname);

            $http({
                method: "GET",
                url: "http://localhost:8080/bmtc/api/GPSData/1?routeNo=" + this.routeName + "&direction=" + this.direction + "&date=" + this.date + "&lat=" + lat + "&lon=" + lon + "&name=" + bname+ "&week=true"
            }).then(function(resBusData) {
    ...
    this.findBusStops = function() {
    	...
    	if(this.routeName!=='K_4'){
    		alert("frequncy of bus is not high !")
    	}else{
    	
        if (this.routeName) {
            $http({
                method: "GET",
                url: "http://localhost:8080/bmtc/api/routeNo/" + this.routeName
            }).then(function(resBusData) {
    ...
    this.select = function() {
        ...
        self.funName = function() {
            if (self.routeName) {
                $http({
                    method: "GET",
                    url: "http://localhost:8080/bmtc/api/GPSData/routeNo/" + self.routeName + "?Date=12345" + "&StartTime=" + self.startTime + "&EndTime=" + self.endTime
                }).then(function(resBusData) {
